...
class Routers {
    ...
    add_router(type, route, router) {
        this.routers.push({route: route, router: router, type: type});
    }
}

const router = new Routers();

router.add_router('get', '/personalbest', (req, res) => {
...
router.add_router('get', '/trackbest', (req, res) => {
...
router.add_router('post', '/personalbest', (req, res) => {
    this.connection.query(`SELECT * FROM personalbest WHERE track=${req.query.track} AND model=${req.query.model} AND guid=${req.query.guid}`, (error, results) => {
        if (results.length > 0) this.connection.query(`UPDATE personalbest SET guid=${req.query.guid} AND laptime=${req.body.laptime} WHERE track='${this.track}' AND model='${req.query.model}'`, (error, results) => {});
...
router.add_router('post', '/trackbest', (req, res) => {
    this.connection.query(`SELECT * FROM trackbest WHERE track=${req.query.track} AND model=${req.query.model}`, (error, results) => {
        if (results.length > 0) this.connection.query(`UPDATE personalbest SET guid=${req.body.guid} AND laptime=${req.body.laptime} WHERE track='${this.track}' AND model='${req.query.model}'`, (error, results) => {});
...
router.add_router('post', '/username', (req, res) => {
